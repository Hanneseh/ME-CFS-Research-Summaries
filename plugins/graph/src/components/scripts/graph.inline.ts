...
function graphSlug(slug) {
  var normalized = simplifySlug(slug || "");
...
function siteHref(slug) {
  ...
  if (!basePath && window.location.pathname.startsWith("/ME-CFS-Research-Summaries")) {
    basePath = "/ME-CFS-Research-Summaries";
  }
  var normalizedSlug = graphSlug(slug);
  var path = normalizedSlug && normalizedSlug !== "index" ? "/" + normalizedSlug + "/" : "/";
  return basePath.replace(/\/$/, "") + path;
}

(function () {
  function getSlugFromUrl() {
    var bodySlug = document.body?.dataset?.slug;
    if (bodySlug) {
      return graphSlug(bodySlug);
    }
  ...
  function initGraph() {
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      var removeSlugs = new Set(
        (config.removeSlugs || []).map(function (slug) {
          return graphSlug(slug);
        }),
      );
      ...
      try {
        ...
        var contentData = dataRaw.content || dataRaw;
        data = new Map();
        for (var key in contentData) {
          var dataSlug = graphSlug(key);
          if (!removeSlugs.has(dataSlug)) {
            data.set(dataSlug, { ...contentData[key], slug: dataSlug });
          }
        }
      } catch (err) {
      ...
      var isThreadPageSlug = function (candidate) {
        return !candidate.startsWith("tags/") && candidate.split("/").length >= 2;
      };

      data.forEach(function (details, source) {
        var outgoing = details.links || [];
        for (var i = 0; i < outgoing.length; i++) {
          var dest = graphSlug(outgoing[i]);
        ...
        if (showTags) {
          ...
          for (var i = 0; i < tags.length; i++) {
            ...
            if (removeTags.indexOf(tag) === -1) {
              ...
              if (!removeSlugs.has(tagSlug) && allTags.indexOf(tagSlug) === -1) {
                allTags.push(tagSlug);
              }
              if (!removeSlugs.has(tagSlug)) {
      ...
      if (slug === "index") {
        validLinks.forEach(function (target) {
          if (target !== "index" && isThreadPageSlug(target)) {
      ...
      if (enableDrag) {
        ...
        var dragEnded = function (event) {
          ...
          if (Date.now() - dragStartTime < 500) {
            var target = siteHref(event.subject.id);
        ...
        for (var i = 0; i < nodeRenderData.length; i++) {
          (function (nodeData) {
            nodeData.gfx.on("click", function () {
              var target = siteHref(nodeData.simulationData.id);
              window.location.href = target;
            });
          })(nodeRenderData[i]);
        }
